...
type ExamLink = { id: number; title: string; lob: Lob | 'All LOBs'; type: string; url: string; notes: string | null; status: 'Active' | 'Archived'; addedBy: string; createdAt: string; createdByUserId: number | null };
...
type Db = {
  ...
  snapshots: Snapshot[];
  users: AuthUser[];
  counters: { agent: number; activity: number; session: number; attendance: number; update: number; batch: number; batchTrainee: number; batchAttendance: number; batchQuiz: number; batchTyping: number; examLink: number; log: number };
};
...
const emptyDb = (): Db => ({
  ...
  counters: { agent: 1, activity: 1, session: 1, attendance: 1, update: 1, batch: 1, batchTrainee: 1, batchAttendance: 1, batchQuiz: 1, batchTyping: 1, examLink: 1, log: 1 },
});
...
function loadDb(): Db {
  try {
    ...
    merged.examLinks = (merged.examLinks ?? []).map((e:any) => ({ ...e, createdByUserId: e.createdByUserId ?? null }));
...
export function useCreateSession(){ return useLocalMutation<any,any>(({data})=>withDb(db=>{ const id=db.counters.session++; const owner=ownershipForCreate(); const s:Session={id,sessionId:`S${String(id).padStart(3,'0')}`,activityId:data.activityId,sessionDate:data.sessionDate,trainerId:data.trainerId,lob:data.lob,type:data.type,topic:data.topic||null,durationMinutes:Number(data.durationMinutes)||0,status:data.status,notes:data.notes||null,...owner};db.sessions.push(s);const activity=db.activities.find(a=>a.id===s.activityId);for(const agentId of activity?.requiredAgentIds??[]){db.attendance.push({id:db.counters.attendance++,sessionId:s.id,agentId,status:'Absent',result:null,notes:null});}log(db,'Session Created',s.sessionId,TRAINERS.find(t=>t.id===s.trainerId)?.name??null);return sessionView(db,s); })); }
...
export function useAddBatchTrainees(){ return useLocalMutation<any,any>(({id,data})=>withDb(db=>{ const b=db.batches.find(x=>x.id===id); if(!b) throw new Error('Batch not found'); const created:BatchTrainee[]=[]; if(b.batchType==='Upskill'){ const existing=new Set(db.batchTrainees.filter(t=>t.batchId===id).map(t=>t.sourceAgentId).filter(Boolean)); const ids=new Set<number>((data.agentIds??[]).map((x:any)=>Number(x))); for(const a of activeAgents(db).filter(a=>a.employmentStatus==='Active'&&ids.has(a.id)&&!existing.has(a.id))){ const t:BatchTrainee={id:db.counters.batchTrainee++,batchId:id,sourceAgentId:a.id,name:a.name,mis:a.mis,hrId:a.hrId,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:a.notes,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}; db.batchTrainees.push(t); created.push(t); } log(db,'Upskill Participants Added',`${b.batchId} · ${created.length} agents`); } else { for(const name of (data.names??[]).map((x:string)=>x.trim()).filter(Boolean)){ const t:BatchTrainee={id:db.counters.batchTrainee++,batchId:id,sourceAgentId:null,name,mis:null,hrId:null,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:null,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}; db.batchTrainees.push(t); created.push(t); } log(db,'New Hires Added',`${b.batchId} · ${created.length} new hires`); } return created; })); }
...
export function useCreateExamLink(){ return useLocalMutation<any,any>(({data})=>withDb(db=>{ let actor='Unknown'; try{actor=JSON.parse(sessionStorage.getItem('keeta-current-user-v1')||'null')?.name||'Unknown';}catch{} const current=currentActor(); const e:ExamLink={id:db.counters.examLink++,title:data.title,lob:data.lob,type:data.type||'Quiz',url:data.url,notes:data.notes||null,status:data.status||'Active',addedBy:actor,createdAt:new Date().toISOString(),createdByUserId:current?.id??null}; db.examLinks.push(e); log(db,'Exam Link Added',e.title); return e; })); }
...
export function useDeleteExamLink(){ return useLocalMutation<any,any>(({id})=>withDb(db=>{ const e=db.examLinks.find(x=>x.id===id); if(!e) throw new Error('Exam link not found'); requireOwnerOrAdmin(e,'exam link'); db.examLinks=db.examLinks.filter(x=>x.id!==id); log(db,'Exam Link Deleted',e.title); return {deleted:true}; })); }
