...
export type BatchAttendanceStatus = 'Not Marked' | 'Attended' | 'Absent' | 'Resigned';
...
type ExamLink = { id: number; itemKind: 'Exam Package' | 'Question Bank'; title: string; lob: Lob | 'All LOBs'; type: string; url: string | null; notes: string | null; status: 'Active' | 'Archived'; version: string; assignedBatchIds: number[]; assignedSessionIds: number[]; addedBy: string; createdAt: string; updatedAt: string; createdByUserId: number | null };
...
function numericPercent(value: any): number | null {
  const raw=String(value ?? '').trim(); if(!raw) return null;
  const frac=raw.match(/^\s*(-?\d+(?:\.\d+)?)\s*\/\s*(-?\d+(?:\.\d+)?)\s*$/);
  if(frac){ const a=Number(frac[1]), b=Number(frac[2]); return b ? (a/b)*100 : null; }
  const n=Number(raw.replace('%','').trim()); return Number.isFinite(n)?n:null;
}
function autoQuizResult(score:any): string { const n=numericPercent(score); return n==null?'Pending':n>60?'Passed':'Failed'; }
function autoTypingResult(wpm:any): string { const n=Number(String(wpm??'').trim()); return !Number.isFinite(n)||String(wpm??'').trim()===''?'Pending':n>=25?'Passed':'Failed'; }
...
function loadDb(): Db {
  try {
    ...
    merged.examLinks = (merged.examLinks ?? []).map((e:any) => ({ ...e, itemKind: e.itemKind === 'Question Bank' ? 'Question Bank' : 'Exam Package', url: e.itemKind === 'Question Bank' ? null : (e.url ?? null), version: e.version ?? '1.0', assignedBatchIds: Array.isArray(e.assignedBatchIds)?e.assignedBatchIds:[], assignedSessionIds: Array.isArray(e.assignedSessionIds)?e.assignedSessionIds:[], updatedAt: e.updatedAt ?? e.createdAt ?? new Date().toISOString(), createdByUserId: e.createdByUserId ?? null }));
    ...
    // Auto-result rules apply to historical/imported data too.
    merged.batchQuiz = merged.batchQuiz.map((q:any)=>({ ...q, result:autoQuizResult(q.score) }));
    merged.batchTyping = merged.batchTyping.map((x:any)=>({ ...x, result:autoTypingResult(x.wpm) }));
    return merged;
  } catch {
    return emptyDb();
  }
...
function batchView(db:Db, b:Batch) {
  ...
  const resignedIds=new Set(db.batchAttendance.filter(a=>a.batchId===b.id&&a.status==='Resigned').map(a=>a.traineeId));
  const currentTrainees=trainees.filter(t=>!resignedIds.has(t.id));
  return { ...b, assistantTrainerIds:b.assistantTrainerIds??[], batchType:b.batchType==='Upskill'?'Upskill':'New Hire', trainer:trainerFor(b.trainerId), traineeCount:currentTrainees.length, totalTraineeCount:trainees.length, resignedCount:resignedIds.size, passedCount:currentTrainees.filter(t=>t.certificationStatus==='Passed').length, nestingW1Count:currentTrainees.filter(t=>t.stage==='Nesting W1').length, nestingW2Count:currentTrainees.filter(t=>t.stage==='Nesting W2').length, activeCount:currentTrainees.filter(t=>t.stage==='Active').length, failedCount:currentTrainees.filter(t=>String(t.stage||'').startsWith('Failed')).length, graduatedCount:currentTrainees.filter(t=>t.graduated).length };
}
...
export function useSaveBatchDayQuiz(){ return useLocalMutation<any,any>(({batchId,dayNumber,date,rows})=>withDb(db=>{ const b=db.batches.find(x=>x.id===batchId); if(!b) throw new Error('Batch not found'); requireBatchManager(b,'save quizzes'); const idx=Number(dayNumber)-1; if(idx>=0&&idx<b.durationDays) b.dayDates[idx]=date; for(const row of rows as any[]){ const resignedDay=db.batchAttendance.filter(a=>a.batchId===batchId&&a.traineeId===row.traineeId&&a.status==='Resigned').reduce((m,a)=>Math.min(m,a.dayNumber),Infinity); if(Number(dayNumber)>resignedDay) continue; let rec=db.batchQuiz.find(q=>q.batchId===batchId&&q.traineeId===row.traineeId&&q.dayNumber===dayNumber); if(rec){rec.score=row.score||null;rec.result=autoQuizResult(row.score);rec.notes=row.notes||null;rec.date=date;} else db.batchQuiz.push({id:db.counters.batchQuiz++,batchId,traineeId:row.traineeId,dayNumber,date,score:row.score||null,result:autoQuizResult(row.score),notes:row.notes||null}); } log(db,'Batch Quiz Updated',`${b.batchId} · Day ${dayNumber}`); return batchDetail(db,batchId); })); }
export function useSaveBatchDayTyping(){ return useLocalMutation<any,any>(({batchId,dayNumber,date,rows})=>withDb(db=>{ const b=db.batches.find(x=>x.id===batchId); if(!b) throw new Error('Batch not found'); requireBatchManager(b,'save typing tests'); const idx=Number(dayNumber)-1; if(idx>=0&&idx<b.durationDays) b.dayDates[idx]=date; for(const row of rows as any[]){ const resignedDay=db.batchAttendance.filter(a=>a.batchId===batchId&&a.traineeId===row.traineeId&&a.status==='Resigned').reduce((m,a)=>Math.min(m,a.dayNumber),Infinity); if(Number(dayNumber)>resignedDay) continue; for(const language of ['Arabic','English'] as TypingLanguage[]){ const src=language==='Arabic'?row.arabic:row.english; let rec=db.batchTyping.find(x=>x.batchId===batchId&&x.traineeId===row.traineeId&&x.dayNumber===dayNumber&&x.language===language); if(rec){rec.wpm=src?.wpm||null;rec.accuracy=src?.accuracy||null;rec.result=autoTypingResult(src?.wpm);rec.notes=src?.notes||null;rec.date=date;} else db.batchTyping.push({id:db.counters.batchTyping++,batchId,traineeId:row.traineeId,dayNumber,date,language,wpm:src?.wpm||null,accuracy:src?.accuracy||null,result:autoTypingResult(src?.wpm),notes:src?.notes||null}); } } log(db,'Batch Typing Test Updated',`${b.batchId} · Day ${dayNumber}`); return batchDetail(db,batchId); })); }
...
export function useCreateExamLink(){ return useLocalMutation<any,any>(({data})=>withDb(db=>{ let actor='Unknown'; try{actor=JSON.parse(sessionStorage.getItem('keeta-current-user-v1')||'null')?.name||'Unknown';}catch{} const current=currentActor(); const kind = data.itemKind === 'Question Bank' ? 'Question Bank' : 'Exam Package'; const now=new Date().toISOString(); const e:ExamLink={id:db.counters.examLink++,itemKind:kind,title:data.title,lob:data.lob,type:data.type||'Quiz',url:kind==='Question Bank'?null:(data.url||null),notes:data.notes||null,status:data.status||'Active',version:data.version||'1.0',assignedBatchIds:Array.isArray(data.assignedBatchIds)?data.assignedBatchIds.map(Number):[],assignedSessionIds:Array.isArray(data.assignedSessionIds)?data.assignedSessionIds.map(Number):[],addedBy:actor,createdAt:now,updatedAt:now,createdByUserId:current?.id??null}; db.examLinks.push(e); log(db,kind==='Question Bank'?'Question Bank Added':'Exam Package Added',e.title); return e; })); }
export function useUpdateExamLink(){ return useLocalMutation<any,any>(({id,data})=>withDb(db=>{ const e=db.examLinks.find(x=>x.id===id); if(!e) throw new Error('Exam item not found'); requireOwnerOrAdmin(e,'exam item'); const nextKind=data.itemKind??e.itemKind; Object.assign(e,data,{itemKind:nextKind,url:nextKind==='Question Bank'?null:(data.url??e.url),version:data.version??e.version??'1.0',assignedBatchIds:Array.isArray(data.assignedBatchIds)?data.assignedBatchIds.map(Number):(e.assignedBatchIds??[]),assignedSessionIds:Array.isArray(data.assignedSessionIds)?data.assignedSessionIds.map(Number):(e.assignedSessionIds??[]),updatedAt:new Date().toISOString()}); log(db,nextKind==='Question Bank'?'Question Bank Updated':'Exam Package Updated',e.title); return {...e}; })); }
