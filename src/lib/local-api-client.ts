...
function syncTraineeStage(db:Db,t:BatchTrainee,b:Batch){
  ...
  if(b.batchType==='New Hire' && ['Nesting W1','Nesting W2','Active'].includes(t.stage||'')){
    if(!t.hrId?.trim()||!t.mis?.trim()) return;
    let a=t.agentId?db.agents.find(x=>x.id===t.agentId):undefined;
    if(!a){
      if(db.agents.some(x=>x.hrId.trim()===t.hrId!.trim() || x.mis.trim().toLowerCase()===t.mis!.trim().toLowerCase())) return;
      a={id:db.counters.agent++,hrId:t.hrId.trim(),mis:t.mis.trim(),name:t.name,lob:b.lob,employmentStatus:'Active',trainingStage:t.stage as any,dateAdded:new Date().toISOString().slice(0,10),notes:t.notes||`From ${b.name}`,archivedAt:null}; db.agents.push(a); t.agentId=a.id; t.graduated=true;
    } else { a.archivedAt=null; a.employmentStatus='Active'; a.trainingStage=t.stage as any; a.name=t.name; a.hrId=t.hrId.trim(); a.mis=t.mis.trim(); }
  }
...
export function useCreateBatch(){ return useLocalMutation<any,any>(({data})=>withDb(db=>{
  ...
  if(batchType==='Upskill'){ const ids=new Set<number>((data.agentIds??[]).map((x:any)=>Number(x))); for(const a of activeAgents(db).filter(a=>a.employmentStatus==='Active'&&ids.has(a.id))) db.batchTrainees.push({id:db.counters.batchTrainee++,batchId:id,sourceAgentId:a.id,name:a.name,mis:a.mis,hrId:a.hrId,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:a.notes,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,knowledgeAttempt1:'Pending',knowledgeAttempt2:'Pending',mockAttempt1:'Pending',mockAttempt2:'Pending',mockCaseId1:null,mockCaseId2:null,nestingWeek1:'Pending',nestingWeek2:'Pending',stage:'Training',finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}); } else { const names=(data.traineeNames??[]).map((x:string)=>x.trim()).filter(Boolean); for(const name of names) db.batchTrainees.push({id:db.counters.batchTrainee++,batchId:id,sourceAgentId:null,name,mis:null,hrId:null,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:null,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,knowledgeAttempt1:'Pending',knowledgeAttempt2:'Pending',mockAttempt1:'Pending',mockAttempt2:'Pending',mockCaseId1:null,mockCaseId2:null,nestingWeek1:'Pending',nestingWeek2:'Pending',stage:'Training',finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}); }
...
export function useAddBatchTrainees(){ return useLocalMutation<any,any>(({id,data})=>withDb(db=>{ const b=db.batches.find(x=>x.id===id); if(!b) throw new Error('Batch not found'); requireBatchManager(b,'add participants'); const created:BatchTrainee[]=[]; if(b.batchType==='Upskill'){ const existing=new Set(db.batchTrainees.filter(t=>t.batchId===id).map(t=>t.sourceAgentId).filter(Boolean)); const ids=new Set<number>((data.agentIds??[]).map((x:any)=>Number(x))); for(const a of activeAgents(db).filter(a=>a.employmentStatus==='Active'&&ids.has(a.id)&&!existing.has(a.id))){ const t:BatchTrainee={id:db.counters.batchTrainee++,batchId:id,sourceAgentId:a.id,name:a.name,mis:a.mis,hrId:a.hrId,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:a.notes,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,knowledgeAttempt1:'Pending',knowledgeAttempt2:'Pending',mockAttempt1:'Pending',mockAttempt2:'Pending',mockCaseId1:null,mockCaseId2:null,nestingWeek1:'Pending',nestingWeek2:'Pending',stage:'Training',finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}; db.batchTrainees.push(t); created.push(t); } log(db,'Upskill Participants Added',`${b.batchId} · ${created.length} agents`); } else { for(const name of (data.names??[]).map((x:string)=>x.trim()).filter(Boolean)){ const t:BatchTrainee={id:db.counters.batchTrainee++,batchId:id,sourceAgentId:null,name,mis:null,hrId:null,jw:null,email:null,portalPassword:null,phoneNumber:null,notes:null,quizScore:null,quizResult:'Pending',quizNotes:null,typingWpm:null,typingAccuracy:null,typingResult:'Pending',typingNotes:null,knowledgeAttempt1:'Pending',knowledgeAttempt2:'Pending',mockAttempt1:'Pending',mockAttempt2:'Pending',mockCaseId1:null,mockCaseId2:null,nestingWeek1:'Pending',nestingWeek2:'Pending',stage:'Training',finalScore:null,finalNotes:null,certificationStatus:'Pending',graduated:false,agentId:null}; db.batchTrainees.push(t); created.push(t); } log(db,'New Hires Added',`${b.batchId} · ${created.length} new hires`); } return created; })); }
